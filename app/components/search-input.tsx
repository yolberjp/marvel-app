...
export default function SearchInput({ name: inputName, ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <div className="relative flex items-center border-b border-gray-600">
      <div className="text-muted-foreground/80 pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-2 peer-disabled:opacity-50">
        {isPending ? <LoaderCircleIcon className="size-5 animate-spin" /> : <SearchIcon className="size-5 text-gray-400" />}
      </div>
      <input type="text" placeholder="search a character..." className="peer ps-10 pe-9 flex-1 p-2 uppercase placeholder:uppercase outline-0" {...props} value={searchValue}
        onChange={(e) => {
